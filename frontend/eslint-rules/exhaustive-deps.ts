...
import { getAdditionalEffectHooksFromSettings } from "./Utils.ts";
    ...
    create(context: Rule.RuleContext) {
        ...
        function visitFunctionWithDependencies(
            ...
            function isStableKnownHookValue(resolved: Scope.Variable): boolean {
                ...
                let callee: Expression | PrivateIdentifier | Super =
                    init.callee;
            ...
            function isInsideEffectCleanup(
                reference: Scope.Reference
            ): boolean {
                            ...
                            if (
                                ...
                                if (declaredDependencyNode.type === "Literal") {
                                    if (
                                        ...
                                        dependencies.has(
                                            declaredDependencyNode.value as string
                                        )
                                    ) {
                                        reportProblem({
                                            node: declaredDependencyNode,
            ...
            // Check for exhaustive-deps-exclude comment
            const excludedDeps = new Set<string>();
            if (declaredDependenciesNode) {
                // Try multiple approaches to find the comment
                const sourceCode = getSourceCode();

                // Approach 1: Comments before deps array
                let comments = sourceCode.getCommentsBefore(
                    declaredDependenciesNode
                );

                // Approach 2: If no comments, try getting all comments in the function
                if (comments.length === 0) {
                    const allComments = sourceCode.getAllComments();
                    // Filter to comments that are near the dependency array
                    comments = allComments.filter(comment => {
                        if (!comment.range || !declaredDependenciesNode.range)
                            return false;
                        // Comment should be within 100 characters before the deps array
                        return (
                            comment.range[1] <
                                declaredDependenciesNode.range[0] &&
                            comment.range[1] >
                                declaredDependenciesNode.range[0] - 100
                        );
                    });
                }
                ...
                for (const comment of comments) {
                    const match = comment.value.match(
                        /exhaustive-deps-exclude:\s*(.+)/
                    );
                    if (match) {
                        // Parse the excluded dependencies
                        const deps = match[1]
                            .split(",")
                            .map(dep => dep.trim())
            ...
            function areDeclaredDepsAlphabetized(): boolean {
                ...
                const declaredDepKeys = declaredDependencies.map(
                    dep => dep.key
                );
            ...
            function formatDependency(path: string): string {
                ...
                for (let i = 0; i < members.length; i++) {
                    if (i !== 0) {
                        ...
                        const isOptional =
                            optionalChains.get(pathSoFar) === true;
                    ...
                    for (const reference of references) {
                        ...
                        while (
                            ...
                            if (maybeCall.type === "CallExpression") {
                                const correspondingStateVariable =
        ...
        function visitCallExpression(node: CallExpression): void {
            ...
            const nodeWithoutNamespace =
                getNodeWithoutReactNamespace(reactiveHook);
            ...
            const declaredDependenciesNode =
                maybeNode &&
                !(
                    maybeNode.type === "Identifier" &&
                    maybeNode.name === "undefined"
                )
                    ? maybeNode
                    : undefined;
            ...
            const isAutoDepsHook =
                options.experimental_autoDependenciesHooks.includes(
                    reactiveHookName
                );
...
function collectRecommendations({
    ...
    // Then add the missing ones at the end, excluding the explicitly excluded deps.
    ...
    const filteredMissingDeps = new Set(
    ...
    return {
        ...
        unnecessaryDependencies,
        duplicateDependencies,
        missingDependencies: filteredMissingDeps,
    };
}
