...
public class DeviceService(ILogger<DeviceService> logger) : IDeviceService
{
        ...
        while (true)
        {
            ...
            var inactiveThreshold =
                ...
                - TimeSpan.FromMilliseconds(
                    INACTIVE_HEARTBEAT_CHECK_INTERVAL_MS + INACTIVE_HEARTBEAT_THRESHOLD_MS
                );
            ...
            foreach (var kvp in _activeDevices.ToArray())
            {
                ...
                logger.LogDebug(
                    $"Removed inactive device {kvp.Value.DeviceGuid} for client {kvp.Key}"
                );

                bool anotherEntryForThisDeviceAvailable = _activeDevices.Values.Any(deviceInfo =>
                    deviceInfo.DeviceGuid == kvp.Value.DeviceGuid
                );
    ...
    {
        logger.LogDebug(
            $"Received heartbeat {message.DeviceStatus} from device {message.Uuid} for client {clientToken}"
        );
        ...
        if (realUserId == null)
        {
            ...
            logger.LogDebug(
                $"No userId found for client token {clientToken}. Heartbeat is not valid."
            );
            return;
        }
        ...
        if (accountIds.Count == 0)
        {
            ...
            logger.LogDebug(
                $"Device with UUID {message.Uuid} not found in the database. Heartbeat is not valid."
            );
            return;
        }
        ...
        lock (_activeDevices)
        {
            if (_activeDevices.TryGetValue(clientToken, out var deviceInfo))
            {
                if (deviceInfo.DeviceGuid != message.Uuid)
                {
                    ...
                    logger.LogDebug(
                        $"Device UUID mismatch for client {clientToken}: expected {deviceInfo.DeviceGuid}, received {message.Uuid}."
                    );
                    return;
                }

                logger.LogDebug(
                    $"Updated heartbeat for device {message.Uuid} for client {clientToken} from {deviceInfo.LastDeviceStatus} to {message.DeviceStatus}"
                );
            ...
            {
                _activeDevices[clientToken] = new RuntimeDeviceInformation
                {
                    DeviceGuid = message.Uuid,
                    LastHeartbeat = DateTime.UtcNow,
                    LastDeviceStatus = message.DeviceStatus,
                };
                logger.LogDebug(
                    $"Registered new device {message.Uuid} for client {clientToken} with status {message.DeviceStatus}"
                );
            }
        }
    ...
    public string GetDeviceStatus(Guid deviceUuid)
    {
        lock (_activeDevices)
        {
            foreach (var kvp in _activeDevices)
            {
                ...
                if (
                    deviceInfo.DeviceGuid == deviceUuid
                    && _webSocketHandler.GetUserIdForClientToken(kvp.Key) != null
                )
                {
    ...
    public void SendDeviceChangedMessage(
        int userId,
        string action,
        Guid deviceUuid,
        string displayName,
    ...
    )
    {
        ...
        {
            logger.LogDebug(
                $"Sending device-changed message (action: {action}) for device {deviceUuid} to client {token}"
            );
            var deviceChangedMessage = new DeviceChangedMessage
            {
                Action = action,
                Device = new DeviceInfo { Uuid = deviceUuid, DisplayName = displayName },
            };
